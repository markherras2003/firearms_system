        ...
        <div class="grid" v-if="canRead">
            <div class="col-12">
                <div class="card">
                    ...
                    <DataTable ref="dt" :value="firearms_data" v-model:selection="selectedFireArms" dataKey="_id"
                        ...
                        <Column field="personnel_id" header="Personnel" :sortable="true"
                            ...
                            <template #body="slotProps">
                                ...
                                <template
                                    v-if="slotProps.data.personnel_id !== undefined && slotProps.data.personnel_id !== null">
                                    {{ slotProps.data.personnel_id }} -
                                    <span
                                        v-if="slotProps.data.personnel !== null && slotProps.data.personnel.fullname !== undefined && slotProps.data.personnel.fullname !== null">{{
                                            slotProps.data.personnel.fullname }}</span>
                                </template>
                                <template v-else>No Personnel</template>
                            </template>
                        </Column>
                        <Column field="firearms_availability" header="Firearms Availability" :sortable="true"
                            headerStyle="width:25%; min-width:10rem;">
                            <template #body="slotProps">
                                <span class="p-column-title">Availability.</span>
                                <span style="text-align: center">{{ slotProps.data.firearms_availability ? 'Yes' : 'No'
                                }}</span>
                            </template>
                        </Column>
                        ...
                        <Column headerStyle="min-width:10rem;">
                            <template #body="slotProps">
                                <Button icon="pi pi-pencil" v-if="canEdit" class="p-button-rounded p-button-success mr-2"
                                    @click="editFireArms(slotProps.data)" />
                    ...
                    <Dialog v-model:visible="firearmDialog" :style="{ width: '600px' }" header="Firearms Details"
                        :modal="true" class="p-fluid">
                        <div class="field">
                            <label for="name">Firearms Serial No.</label>
                            <InputText id="firearms_serialno" v-model.trim="firearm.firearms_serialno" required="true"
                                autofocus :class="{ 'p-invalid': submitted && !firearm.firearms_serialno }" />
                            <small class="p-invalid" v-if="submitted && !firearm.firearms_serialno">Firearms Serial No. is
                                required.</small>
                        </div>

                        <div class="field">
                            <label for="firearm.firearms">Firearms Type</label>
                            <Dropdown v-model="firearm.firearms" :options="firearms_listdata" optionLabel="firearms"
                                placeholder="Select Firearms Type"
                                :class="{ 'p-invalid': submitted && !firearm.firearms }" />
                        ...
                        <div class="field">
                            ...
                            <InputText v-model="firearm.firearms_buttnumber" required="true" rows="3" cols="20"
                                :class="{ 'p-invalid': submitted && !firearm.firearms_buttnumber }" />
                            <small class="p-invalid" v-if="submitted && !firearm.firearms_buttnumber">firearms_buttnumber is
                                required.</small>
                        </div>

                        <div class="field">
                            <label for="firearms_qrcode">Firearms QR Code</label>
                            <InputText id="firearms_qrcode" v-model.trim="firearm.firearms_qrcode" required="true" rows="3"
                                cols="20" :class="{ 'p-invalid': submitted && !firearm.firearms_qrcode }" />
                            <small class="p-invalid" v-if="submitted && !firearm.firearms_qrcode">Fire Arms QR Code is
                                Required.</small>
                        </div>
                        ...
                        <div class="field" style="margin-top: 10px">
                            <InputSwitch v-model="firearm.firearms_isperson" default: />
                        </div>
                        ...
                        <div class="field" v-if="firearm.firearms_isperson">
                            ...
                            <AutoComplete placeholder="Search Personnel" id="dd" :dropdown="true" :multiple="false"
                                v-model="firearm.personnel_id" :options="personneldata" :suggestions="autoFilteredValue"
